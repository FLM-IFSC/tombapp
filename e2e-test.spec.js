...
test.describe('Full application workflow', () => {
  ...
  test('should correctly apply all status actions', async ({ page }) => {
    await page.goto('file://' + path.resolve(__dirname, 'index.html'));
    await page.setInputFiles('input#csvFileInput', csvPath);
    await expect(page.locator('#mainContent')).toBeVisible({ timeout: 10000 });

    const actions = [
      { tombo: 'T001', action: 'found', expectedStatus: 'Encontrado' },
      { tombo: 'T002', action: 'not-found', expectedStatus: 'Não Encontrado' },
      { tombo: 'T004', action: 'dispose', expectedStatus: 'Desfazimento Solicitado' },
      { tombo: 'T006', action: 'no-tombo', expectedStatus: 'Tombo Inválido' },
      { tombo: 'T007', action: 'request-label', expectedStatus: 'Etiqueta Solicitada' },
    ];

    for (const { tombo, action, expectedStatus } of actions) {
      // Search for the item
      await page.locator('#tomboInput').fill(tombo);
      await page.locator('#loadButton').click();

      // Click the action button
      await page.locator(`button[data-action="${action}"]`).click();

      // Verify status update in details view
      await expect(page.locator('#itemStatus')).toHaveText(`Status: ${expectedStatus}`);

      // Verify status update in the main table
      const row = page.locator(`#mainTableBody tr[data-tombo='${tombo}']`);
      await expect(row.locator('td .status-badge')).toHaveText(expectedStatus);
    }
  });
});