document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleAction(action) {
        ...
        switch (action) {
            ...
            case 'dispose':
                ...
                break;
            case 'no-tombo':
                newStatus = 'Tombo Inválido';
                toastMessage = 'Item marcado como Tombo Inválido.';
                break;
            case 'request-label':
                newStatus = 'Etiqueta Solicitada';
                toastMessage = 'Solicitação de nova etiqueta registrada.';
                break;
            default:
    ...
    // Adiciona um único event listener ao documento para capturar todos os cliques.
    // Isso usa "event delegation" da forma mais robusta possível.
    document.addEventListener('click', (event) => {
        const button = event.target.closest('.action-button[data-action]');
        if (button) {
            // Garante que a ação só seja executada se o botão estiver dentro da seção de detalhes visível.
            if (document.getElementById('itemDetailsSection').contains(button)) {
